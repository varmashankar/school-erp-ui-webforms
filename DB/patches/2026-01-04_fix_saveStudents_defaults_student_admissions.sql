-- Patch: fix dbo.saveStudents defaults and student_admissions insert
-- Reason:
-- 1) Draft admission flow can call saveStudents with NULL @userId/@status.
-- 2) student_admissions.created_by_id is NOT NULL, so inserting NULL fails.
-- 3) Update branch uses @outputId before it is set, causing wrong linkage.
--
-- Apply this after importing/creating the DB from schoolfullerp.sql.
...
BEGIN TRY
    ...
    -- Preserve existing convention (script currently uses 4)
    IF (@userId IS NULL)
        SET @userId = 4;
    ...
    BEGIN
        ...
        -----------------------------------------------------------------
        -- UPSERT INTO STUDENT_ADMISSIONS
        -----------------------------------------------------------------
    ...
    BEGIN
        ...
        -----------------------------------------------------------------
        -- UPSERT INTO STUDENT_ADMISSIONS (use @id / @outputId correctly)
        -----------------------------------------------------------------
